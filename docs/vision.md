...
도메인 맥락은 dataset card에서 설명한다.

### 하나의 config = 하나의 dataset

YAML config 하나가 게시될 내용을 완전히 정의한다: source, 컬럼 매핑, 타입, 필터, 출력.
선언되지 않은 컬럼도 없고, 예상 밖의 결과도 없다. Config는 단일 진실 공급원이다.

### 검증 게이트

출력 스키마가 config에서 벗어나면 게시가 실패한다. 구체적으로는 다음과 같다:
...
| **kpubdata-builder** | 빌드 파이프라인 — fetch, transform, validate, publish 담당 |
| **kpubdata-studio** | 시각적 워크벤치 UI(향후) — inspect, preview, export 담당 |
