...
권고안(대안 B, SQLite 인덱스)을 **수정 채택**한다. 단, 적용 범위와 안전 장치를 명확히 한정한다.

1. SQLite(표준 라이브러리 `sqlite3`, 무외부의존)는 **완료된 빌드의 파생·재구축 가능한 인덱스**로만 사용한다(목록·조회). `manifest.json`이 여전히 정본이다.
2. **비동기 job 상태(`queued`/`running`)는 별도 관심사**다. 빌드 진행 중에는 manifest가 없으므로 이 인덱스로 표현하지 않는다. 이 ADR은 **완료된 빌드 이력**에만 한정되며, ADR 0002 async가 구현될 때 job-state 저장은 자체 설계를 가진다.
3. 안전 요구사항(구현 시 강제): SQLite `busy_timeout` 설정, WAL 모드, 모든 쓰기에 트랜잭션, **스키마 버전 컬럼**, 인덱스 재구축(rebuild) 커맨드.
4. **인덱스 쓰기 실패가 성공한 빌드를 실패시켜서는 안 된다.** 인덱스 갱신은 best-effort이며, 유실 시 스캔으로 재구축 가능하다(권위 없음).

> 근거: manifest 정본 유지는 AGENTS.md '매니페스트 누락 금지'와 정합하며, 인덱스를 파생물로 둔 것은 결정적·재현 가능성을 보존한다.
